...
export default function LogFeeding() {
  ...
  const {
    ...
    defaultValues: {
      items: [{ type: '', amount: '', unit: baby?.unitPreference ?? 'oz' }],
  ...
  const isBreast = watchedItems?.some((item) => BREAST_TYPES.has(item.type))
  ...
  useEffect(() => {
    ...
    getFeeding(user.uid, feedingId).then((f) => {
      ...
      const dt = f.startTime?.toDate ? f.startTime.toDate() : new Date(f.startTime)
      const loadedItems = f.items ?? [{ type: f.type, amount: f.amount ?? '', unit: f.unit ?? baby?.unitPreference ?? 'oz' }]
      reset({
        items: loadedItems.map((i) => ({
          type: i.type,
          amount: i.amount ?? '',
          unit: i.unit ?? baby?.unitPreference ?? 'oz',
        })),
  ...
  }, [isEdit, feedingId, user, navigate, reset, baby?.unitPreference])
      ...
      <div className="space-y-3">
          ...
          <div key={field.id} className="space-y-4 p-4 bg-gray-50 rounded-2xl relative">
            ...
            <div>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <FeedingTypeSelector value={field.value} onChange={field.onChange} />
                )}
              />
            ...
            <div>
              <label className="label">
                Amount{BREAST_TYPES.has(watchedItems?.[index]?.type) ? ' (optional)' : ''}
              </label>
              <Controller
        ...
        <button
          ...
          onClick={() => append({
            type: '',
            amount: '',
            unit: watchedItems?.[watchedItems.length - 1]?.unit ?? baby?.unitPreference ?? 'oz',
          })}

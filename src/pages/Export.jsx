...
export default function Export() {
  ...
  const avgAmount = useMemo(() => {
    const withAmt = filtered.filter((f) => f.amount > 0)
    if (!withAmt.length) return null
    const sum = withAmt.reduce((a, f) => a + f.amount, 0)
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-gray-800">
              {avgAmount != null ? formatAmount(avgAmount, baby?.unitPreference ?? 'oz') : '—'}
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-2xl font-extrabold text-gray-800">
              {filtered.length > 0
                ? (filtered.length / Math.max(1, Math.ceil((new Date(dateTo) - new Date(dateFrom)) / 86400000) + 1)).toFixed(1)
                : '—'}
            </p>
          </div>
        </div>

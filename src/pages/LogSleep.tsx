...
export default function LogSleep() {
  ...
  useEffect(() => {
    ...
    getSleep(user.uid, sleepId!).then((s) => {
      ...
      const start = s.startTime?.toDate ? s.startTime.toDate() : new Date(s.startTime as unknown as string)
      const end = s.endTime ? (s.endTime.toDate ? s.endTime.toDate() : new Date(s.endTime as unknown as string)) : null
      reset({
  ...
  const onSubmit: SubmitHandler<LogSleepFormValues> = async (data) => {

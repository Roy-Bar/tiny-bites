...
import type { Feeding, FeedingTypeDef, TimestampLike } from '../../types'
    ...
    <div className="card p-4">
      ...
      >
        ...
        {todayFeedings.map((f) => {
          ...
          const items: { type: string }[] = f.items?.filter((i) => i.type) ?? [{ type: f.type }]
          const isMixed = items.length > 1
          let dotStyle: CSSProperties
          if (isMixed) {
            const colors = items.map((i) => (FEEDING_TYPE_MAP[i.type] ?? {} as Partial<FeedingTypeDef>).color ?? '#f96b3a')
            const step = 100 / colors.length
            const stops = colors.map((c, i) => `${c} ${i * step}% ${(i + 1) * step}%`).join(', ')
            dotStyle = { background: `conic-gradient(${stops})` }
          } else {
            const typeInfo = FEEDING_TYPE_MAP[items[0]?.type ?? f.type] ?? {} as Partial<FeedingTypeDef>

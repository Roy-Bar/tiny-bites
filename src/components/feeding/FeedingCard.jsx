...
export default function FeedingCard({ feeding, showDate = false }) {
  const user = useAuth()
  const toast = useToast()
  const [confirmOpen, setConfirmOpen] = useState(false)

  const items = feeding.items?.length > 0
    ? feeding.items
    : [{ type: feeding.type, amount: feeding.amount, unit: feeding.unit }]
  const typeInfo = FEEDING_TYPE_MAP[items[0]?.type] ?? {}
      ...
      <div className="card p-4 flex items-start gap-4 animate-fade-in">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {items.map((item, i) => {
              const info = FEEDING_TYPE_MAP[item.type] ?? {}
              return (

...
import { MoonIcon, SleepingIcon, SunIcon } from './icons'

interface SleepTrackerProps {
  hideViewAll?: boolean
}

export default function SleepTracker({ hideViewAll = false }: SleepTrackerProps) {
    ...
    <div className="card p-5 bg-gradient-to-br from-lavender-50 to-peach-50 border-lavender-100 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 py-2">
          <p className="flex items-center gap-1.5 text-sm font-semibold text-lavender-500">
            <SleepingIcon size={18} /> Sleeping
          </p>
          ...
          <button
            onClick={wakeUp}
            disabled={busy}
            className="btn-lavender w-full mt-1"
          >
            {busy ? <Spinner size="sm" /> : <><SunIcon size={18} /> Wake Up</>}
          </button>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          <button
            onClick={startSleep}
            disabled={busy}
            className="btn-lavender w-full text-sm"
          >
            {busy ? <Spinner size="sm" /> : <><MoonIcon size={18} /> Start Sleep</>}
          </button>
        </div>
      )}
